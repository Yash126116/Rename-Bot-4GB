...
def insert(chat_id):
    ...
    user_det = {"_id": user_id, "file_id": None, "caption": None, "daily": 0, "date": 0,
                "uploadlimit": 10995116277760, "used_limit": 0, "usertype": "Free", "prexdate": None,
                "metadata": False, "metadata_code": ""}
    try:
...
def set_metadata(user_id, bool_meta):
    dbcol.update_one({"_id": user_id}, {"$set": {"metadata": bool_meta}})

def get_metadata_code(user_id):
    user = dbcol.find_one({"_id": user_id})
    return user.get("metadata_code", "")

def set_metadata_code(user_id, metadata_code):
    dbcol.update_one({"_id": user_id}, {"$set": {"metadata_code": metadata_code}})